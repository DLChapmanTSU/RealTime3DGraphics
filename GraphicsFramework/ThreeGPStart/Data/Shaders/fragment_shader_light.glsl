...
float IsInShadow(int light)
{
	vec4 fragInLightSpace = lights[light].m_lightSpaceMatrix * vec4(varying_position.xyz, 1.0f);
	vec3 projectionCoords = fragInLightSpace.xyz / fragInLightSpace.w;
	projectionCoords = (projectionCoords + 1.0) / 2.0;
	float currentDepth = projectionCoords.z;
	if (currentDepth > 1.0f)
		return 0.0f;
	float closestDepth = texture2D(lights[light].m_shadowMap, projectionCoords.xy).r;
	//bool isInShadow = currentDepth - shadowBias > closestDepth;
	//if (isInShadow)
		//return 1.0f;
	
	float shadow = 0.0f;
	vec2 texelSize = 1.0 / textureSize(lights[light].m_shadowMap, 0);
	for(int x = -1; x <= 1; ++x)
	{
		for(int y = -1; y <= 1; ++y)
		{
		    float pcfDepth = texture2D(lights[light].m_shadowMap, projectionCoords.xy + vec2(x, y) * texelSize).r; 
...
void main(void)
{
	...
	for (int i = 0; i < 10; i++) {
		...
		if (currentType == 0) {
			...
			sumOfColours += (((diffuse_intensity * tex_colour) + specular) * lights[i].m_colour) * (1.0f - shadow);
		}
			...
			if (angle <= lights[i].m_angle && angle >= -lights[i].m_angle) {
				...
				sumOfColours += (((intensity * tex_colour) + specular) * lights[i].m_colour) * (1.0f - shadow);
			}
		}
